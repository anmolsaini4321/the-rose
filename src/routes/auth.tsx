...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Enter your private atelier to save bouquets and manage your correspondence.",
      },
...
async function getRedirectPath(userId: string): Promise<string> {
  ...
  const isSuperAdmin = (data ?? []).some((r: { role: string }) => r.role === "super_admin");
  return isSuperAdmin ? "/admin" : "/my-bouquets";
}
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (mode === "signup") {
      ...
      if (error) {
        ...
        toast.success("Welcome. Please check your inbox to confirm.");
        const path = data.user ? await getRedirectPath(data.user.id) : "/my-bouquets";
        navigate({ to: path });
      }
    } else {
    ...
    <div className="min-h-screen bg-cream flex flex-col">
      ...
      <main className="flex-1 flex items-center justify-center px-6 py-32">
        <div className="w-full max-w-md bg-ivory border border-border p-10">
          ...
          <h1 className="text-display text-4xl mt-4 text-center">
            {mode === "signin" ? "Welcome back." : "Begin your atelier."}
          </h1>
          <p className="text-center text-sm opacity-70 mt-3">
            {mode === "signin"
              ? "Continue composing your bouquets."
              : "Save designs, publish to the gallery."}
          </p>

          <button
            onClick={google}
            className="mt-8 w-full border border-border py-3 flex items-center justify-center gap-3 hover:bg-cream transition text-eyebrow"
          >
